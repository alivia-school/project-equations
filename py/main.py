...
def do(e):
    ...
    # Четвертая стадия    
        ...
        if needCalcUnknown(eq):
            ...
            # Не нужно упрощать
            note.text += "В данном случае вычисление не требуется. Нажмите далее для перехода к следующему этапу"
            result = True
    ...
    if result == False:
        # Уравнение не имеет корней. Закончим вычисления
        # Пояснение
        note.text = "На данном этапе выяснилось, что уравнение не имеет корней. Молодец, уравнение завершено."
        # Спрячем поле для ответа, оно нам более не нужно
        ce.hide()
        # Кнопка "Далее" не нужна более
        button_do.hide()              
